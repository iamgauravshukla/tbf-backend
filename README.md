...
## Moving to Postgres (Railway)

All storage goes through **`src/lib/db.js`** (a tiny JSON-collection engine) and the
modules on top of it — `store.js` (leads), `users.js`, `activity.js`, `tasks.js`.
Reimplement `db.js` (or those modules) against Postgres and nothing above changes:
the routes call the same functions. Suggested tables:

- **leads** — `id, received_at, updated_at, status, assigned_to, appointment_at, notes,
  name, email, phone, treatment, message, source, consent, ip, user_agent`
- **users** — `id, name, email, role, active, password_hash, created_at, updated_at`
  (`password_hash` is scrypt today; keep scrypt or switch to bcrypt/argon2)
- **activity** — `id, lead_id, type, message, actor_id, actor_name, meta, at`
- **tasks** — `id, title, lead_id, lead_name, assigned_to, due_at, done, completed_at,
  created_by, created_at, updated_at`

Add indexes on `leads.assigned_to`, `leads.status`, `activity.lead_id`,
`tasks.assigned_to` and `tasks.due_at`.
